...
from src.strategies.custom.rolling_window import rw_top, rw_bottom
...
def plot_hs(candle_data: pd.DataFrame, pat: HSPattern, pad: int = 2):
    ...
    # plt.show()  # Comment out to avoid blocking in headless environment
    plt.savefig('head_shoulders_pattern.png')
    plt.close()
